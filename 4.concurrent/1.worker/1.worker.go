...
func workerMysql(workerEntry worker) {
	sql := "SELECT * FROM order_goods_instalment where 'id' = %d"
	sql = fmt.Sprintf(sql, workerEntry.page)
	log.Println(sql)
...
//http://192.168.1.177:1234/get_mysql
//计算总时间
//限制并发总数
func main() {
	goroutine_num()
	...
	r.GET("/get_mysql", func(c *gin.Context) {
		var workers [MAX_GOROUTINES]worker
		var results [MAX_GOROUTINES]string
